...
- All *_zh fields MUST be written in Traditional Chinese (繁體中文). 绝对不能用英文写 _zh 字段的内容。Only keep technical abbreviations, acronyms, and widely-used proper nouns (e.g. "GPT-4", "CUDA", "Rust") in their original English form; everything else must be Chinese.
...
Respond with valid JSON only. Each _en field must be in English; each _zh field MUST be in Traditional Chinese (中文). Every field MUST be at least one complete sentence (except community_discussion fields when no comments exist):
{{
